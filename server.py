...
<head>
  ...
  <style>""" + APPROVED_MARKETING_STYLE + r"""
    .fb-hero { position:relative; overflow:hidden; border-radius:18px; padding:22px; margin-bottom:18px; color:white; background:linear-gradient(135deg,#1877f2 0%,#0f766e 45%,#7c3aed 100%); box-shadow:0 18px 44px rgba(24,119,242,.22); }
    .fb-hero:after { content:""; position:absolute; right:-70px; top:-90px; width:250px; height:250px; border-radius:999px; background:rgba(255,255,255,.18); }
    .fb-hero h1 { margin:0; font-size:30px; }
    .fb-hero p { margin:8px 0 0; max-width:780px; color:rgba(255,255,255,.88); line-height:1.45; }
    .fb-quick { display:grid; grid-template-columns:repeat(6,minmax(120px,1fr)); gap:10px; margin-top:18px; position:relative; z-index:1; }
    .fb-quick button, .fb-quick a { min-height:56px; border-radius:14px; padding:10px 12px; color:#172026; background:#fff; box-shadow:0 10px 24px rgba(0,0,0,.14); text-decoration:none; display:flex; align-items:center; justify-content:center; text-align:center; font-weight:800; }
    .fb-quick .primary { color:white; background:linear-gradient(135deg,#16a34a,#0f766e); }
    .fb-quick .blue { color:white; background:linear-gradient(135deg,#1877f2,#2563eb); }
    .fb-quick .violet { color:white; background:linear-gradient(135deg,#7c3aed,#db2777); }
    .fb-card-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-bottom:18px; }
    .fb-stat { background:rgba(255,255,255,.95); border:1px solid var(--line); border-radius:14px; padding:14px; box-shadow:var(--shadow); }
    .fb-stat strong { display:block; font-size:22px; margin-bottom:4px; color:#172026; }
    .fb-stat span { color:var(--muted); font-size:13px; }
    .section { border-radius:14px; }
    .section-header { background:linear-gradient(180deg,#ffffff,#f8fafc); }
    .section-header h2 { font-size:19px; }
    button { border-radius:11px; min-height:42px; }
    button.primary { box-shadow:0 10px 18px rgba(15,118,110,.18); }
    button.danger { color:white; background:linear-gradient(135deg,#ef4444,#b42318); }
    button.ghost { background:#fff; border:1px solid var(--line); }
    .item { border-radius:12px; background:#fff; }
    .item.off { opacity:.62; }
    .tag { background:#e8f2ff; color:#175cd3; }
    .tag.minus { background:#fff1f3; color:#b42318; }
    .fb-actions-row { display:flex; flex-wrap:wrap; gap:8px; margin-top:10px; }
    .fb-send-box { margin-top:18px; padding:18px; background:#fff; border:1px solid var(--line); border-left:8px solid #1877f2; box-shadow:var(--shadow); border-radius:16px; display:grid; grid-template-columns:1fr auto; align-items:center; gap:16px; }
    .fb-send-box strong { font-size:18px; }
    .fb-send-box span { display:block; margin-top:4px; color:var(--muted); font-size:13px; }
    .fb-send-controls { display:flex; gap:8px; flex-wrap:wrap; justify-content:flex-end; }
    .fb-send-controls select { min-width:210px; }
    @media (max-width:1100px) { .fb-quick { grid-template-columns:repeat(3,minmax(0,1fr)); } .fb-card-grid { grid-template-columns:1fr; } .fb-send-box { grid-template-columns:1fr; } .fb-send-controls { justify-content:stretch; } .fb-send-controls select, .fb-send-controls button { width:100%; } }
    @media (max-width:640px) { .fb-quick { grid-template-columns:1fr; } .fb-hero { padding:18px; } }
  </style>
</head>
<body class="locked">
  ...
  <main>
    <div class="fb-hero">
      <h1>CollectorHub · Реклама Facebook</h1>
      <p>Рабочая панель внутри старой кнопки «Реклама Facebook»: группы поиска, ключевые слова, слова-исключения, куда пересылать, рекламные материалы, расписание, найденные объявления и журнал.</p>
      <div class="fb-quick">
        <a class="primary" href="#facebookTargetsPanel">➕ Группы Facebook</a>
        <a class="blue" href="#facebookSettingsPanel">📤 Куда пересылать</a>
        <a href="#messagePanel">📝 Материалы</a>
        <a href="#schedulePanel">⏰ Расписание</a>
        <a class="violet" href="#facebookHitsPanel">🔎 Найденные</a>
        <button onclick="sendNow()">🚀 Отправить сейчас</button>
      </div>
    </div>

    <div class="fb-card-grid">
      <div class="fb-stat"><strong id="targetCount">0 групп</strong><span>Facebook-группы для поиска и рекламы</span></div>
      <div class="fb-stat"><strong id="messageCount">0 материалов</strong><span>готовые тексты и картинки</span></div>
      <div class="fb-stat"><strong id="scheduleCount">0 расписаний</strong><span>автоматическая отправка</span></div>
    </div>

    <div class="content"><div>
      <section class="section" id="facebookSettingsPanel"><div class="section-header"><div><h2>📤 Мои группы для пересылки</h2><p>Куда перенаправлять найденные объявления из Facebook.</p></div></div><div class="section-body"><form class="form-grid" onsubmit="saveFacebookSettings(event)"><label>Моя группа Telegram<select id="fbForwardTelegramChatId"></select></label><label>Facebook ID / PSID моей группы<input id="fbForwardTargetId" placeholder="Facebook ID"></label><label class="full">Page Access Token<input id="fbPageAccessToken" type="password" placeholder="Вставьте токен, чтобы сохранить или обновить."></label><div class="full meta" id="facebookSettingsSaved"></div><div class="form-actions full"><button class="primary">💾 Сохранить настройки Facebook</button></div></form></div></section>

      <section class="section" id="facebookTargetsPanel"><div class="section-header"><div><h2>📘 Facebook-группы</h2><p>Группы или страницы для планирования рекламы и поиска по ключевым словам.</p></div></div><div class="section-body"><form class="form-grid" onsubmit="saveFacebookTarget(event)"><input id="fbTargetId" type="hidden"><label>Название<input id="fbTargetName" required placeholder="Например: Работа Świdnica"></label><label>ID или ссылка<input id="fbTargetLink" placeholder="ID группы или ссылка"></label><label>Действие при совпадении<select id="fbAction"><option value="same_group">Не пересылать, ответить в этой же группе</option><option value="manual">Только записать в журнал</option><option value="forward_telegram">Пересылать в мою группу Telegram</option><option value="forward_facebook">Пересылать в мою группу Facebook</option></select></label><label>Материал для комментария<select id="fbMaterial"></select></label><label class="full">Ключевые слова<input id="fbKeywords" placeholder="работа, квартира, продажа"></label><label class="full">Слова-исключения<input id="fbExcludeKeywords" placeholder="спам, казино, бесплатно"></label><label class="full">Заметки<textarea id="fbNotes" placeholder="Город, что проверять, важные комментарии"></textarea></label><label class="full"><input id="fbEnabled" type="checkbox" checked> Группа включена</label><div class="form-actions full"><button class="primary">✅ Сохранить группу</button><button type="button" class="ghost" onclick="resetFacebookTargetForm()">Очистить форму</button></div></form><div class="items" id="targetsList"></div></div></section>
      ...
      <section class="section" id="schedulePanel"><div class="section-header"><div><h2>⏰ Расписание рекламы</h2><p>Выберите Facebook-группу, материал и время публикации.</p></div></div><div class="section-body"><form class="form-grid" onsubmit="saveSchedule(event)"><input id="scheduleId" type="hidden"><label>Группа<select id="scheduleTarget"></select></label><label>Материал<select id="scheduleMessage"></select></label><label>Время<input id="scheduleTime" type="time" value="10:00" required></label><label><input id="scheduleEnabled" type="checkbox" checked> Расписание включено</label><div class="form-actions full"><button class="primary">💾 Сохранить расписание</button></div></form><div class="items" id="schedulesList"></div></div></section>
    </div><aside>
      <section class="section" id="facebookHitsPanel"><div class="section-header"><div><h2>🔎 Найденные объявления</h2><p>Последние совпадения по ключевым словам Facebook.</p></div><button class="danger" onclick="clearHits()">Очистить найденные</button></div><div class="section-body"><div class="items" id="hitsList"></div></div></section>
      <section class="section"><div class="section-header"><div><h2>📜 Журнал Facebook</h2><p>Последние действия.</p></div><button class="danger" onclick="clearLogs()">Очистка журнала</button></div><div class="section-body"><div class="items" id="logs"></div></div></section>
    </aside></div>

    <div class="fb-send-box"><div><strong>🚀 Отправить сейчас</strong><span>Ручная подготовка выбранного рекламного материала.</span></div><div class="fb-send-controls"><select id="sendTarget"></select><select id="sendMessage"></select><button class="primary" onclick="sendNow()">Отправить сейчас</button></div></div>
  </main>
  <script>const platform = "facebook";""" + APPROVED_MARKETING_SCRIPT + r"""
    function resetFacebookTargetForm() {
      fbTargetId.value = ""; fbTargetName.value = ""; fbTargetLink.value = ""; fbKeywords.value = ""; fbExcludeKeywords.value = ""; fbNotes.value = ""; fbEnabled.checked = true; fbAction.value = "same_group"; fbMaterial.value = "";
    }
    function renderFacebookSettings() {
      ...
      fbForwardTargetId.value = (state.settings && state.settings.facebook_forward_target_id) || "";
      const tg = (state.telegramGroups || []).find(item => String(item.chat_id) === String(selectedTelegram));
    ...
    function actionLabel(value) {
      if (value === "forward_telegram") return "📤 Telegram";
      if (value === "forward_facebook") return "📘 Facebook";
      if (value === "manual") return "📜 Только журнал";
      return "💬 Ответ в группе";
    }
    function render() {
      renderFacebookSettings();
      renderCommonSelects(); renderMessages(); renderSchedules(); renderLogs();
      const targets = state.targets || [];
      targetCount.textContent = mtCount(targets.length, "групп");
      messageCount.textContent = mtCount((state.messages || []).length, "материалов");
      scheduleCount.textContent = mtCount((state.schedules || []).length, "расписаний");
      hitsList.innerHTML = (state.hits || []).map(item => `<article class="item"><strong>${esc(item.keyword)}</strong><div class="meta">${fmt(item.created_at)} · ${esc(item.target_name || item.username || "")}</div><p class="no-translate">${esc(item.message || "")}</p></article>`).join("") || `<div class="empty">Совпадений пока нет</div>`;
      targetsList.innerHTML = targets.map(item => `<article class="item ${item.enabled ? "" : "off"}"><div class="item-head"><div class="item-title"><strong class="no-translate">${esc(item.name)}</strong><span class="no-translate">${esc(item.target_id || "ID/ссылка не указаны")}</span></div><div class="item-actions"><button onclick="editFacebookTarget(${item.id})">✏️ Редактировать</button><button class="danger" onclick="deleteItem('facebook-target', ${item.id})">Удалить</button></div></div><div>${String(item.keywords || "").split(",").filter(Boolean).map(word => `<span class="tag no-translate">${esc(word.trim())}</span>`).join("") || `<span class="meta">Ключевые слова не заданы</span>`}</div><div>${String(item.exclude_keywords || "").split(",").filter(Boolean).map(word => `<span class="tag minus no-translate">-${esc(word.trim())}</span>`).join("")}</div><div class="meta">Материал: ${esc(item.response_message_id || "не выбран")} · действие: ${esc(actionLabel(item.action || "same_group"))}</div><div class="meta no-translate">${esc(item.notes || "")}</div></article>`).join("") || `<div class="empty">Пока нет Facebook-групп</div>`;
      applyMarketingLanguage();
    }
    requireAdminAccess(loadState);
  </script>
