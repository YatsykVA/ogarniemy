...
<head>
  ...
  <style>""" + MARKETING_PAGE_STYLE + r"""
    .fb-hero { position:relative; overflow:hidden; border-radius:18px; padding:22px; margin-bottom:18px; color:white; background:linear-gradient(135deg,#1877f2 0%,#0f766e 50%,#7c3aed 100%); box-shadow:0 18px 44px rgba(24,119,242,.22); }
    .fb-hero h1 { margin:0; font-size:30px; }
    .fb-hero p { margin:8px 0 0; max-width:860px; color:rgba(255,255,255,.9); line-height:1.45; }
    .fb-quick { display:grid; grid-template-columns:repeat(5,minmax(120px,1fr)); gap:10px; margin-top:18px; }
    .fb-quick a, .fb-quick button { min-height:54px; border-radius:14px; padding:10px 12px; color:#172026; background:#fff; box-shadow:0 10px 24px rgba(0,0,0,.14); text-decoration:none; display:flex; align-items:center; justify-content:center; text-align:center; font-weight:800; border:0; cursor:pointer; }
    ...
    .fb-stats { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-bottom:18px; }
    .fb-stat { background:#fff; border:1px solid var(--line); border-radius:14px; padding:14px; box-shadow:var(--shadow); }
    .fb-stat strong { display:block; font-size:22px; margin-bottom:4px; color:#172026; }
    .fb-stat span { color:var(--muted); font-size:13px; }
    .tag { display:inline-flex; align-items:center; margin:4px 4px 0 0; padding:5px 8px; border-radius:999px; background:#e8f2ff; color:#175cd3; font-size:12px; font-weight:800; }
    ...
    button.ghost { background:#fff; border:1px solid var(--line); color:#172026; }
    @media (max-width:1100px) { .fb-quick { grid-template-columns:repeat(2,minmax(0,1fr)); } .fb-stats { grid-template-columns:1fr; } }
    @media (max-width:640px) { .fb-quick { grid-template-columns:1fr; } .fb-hero { padding:18px; } .item-head { display:block; } }
  </style>
</head>
<body class="locked">
  <header>
  ...
  <main>
    <div class="fb-hero">
      <h1>CollectorHub · Facebook</h1>
      <p>Панель внутри старого пункта «Реклама Facebook»: группы, ключевые слова, слова-исключения, материалы, расписание, найденные заявки и журнал. Остальной сервер не трогаем.</p>
      <div class="fb-quick">
        <a class="primary" href="#targetPanel">➕ Группы Facebook</a>
        <a class="blue" href="#messagePanel">📝 Материалы</a>
        <a href="#schedulePanel">⏰ Расписание</a>
        <a class="violet" href="#hitsPanel">🔎 Найденные</a>
        <button onclick="sendNow()">🚀 Отправить сейчас</button>
      </div>
    </div>

    <div class="fb-stats">
      <div class="fb-stat"><strong id="targetCount">0</strong><span>Facebook-групп</span></div>
      <div class="fb-stat"><strong id="messageCount">0</strong><span>рекламных материалов</span></div>
      <div class="fb-stat"><strong id="scheduleCount">0</strong><span>расписаний</span></div>
    </div>

    <section class="grid">
      <div class="panel"><h2>Города</h2><form onsubmit="saveCity(event)"><input id="cityName" placeholder="Świdnica / Wałbrzych" required><button>Добавить город</button></form><div id="cities" class="cards"></div></div>

      <div class="panel" id="targetPanel"><h2>Facebook-группы</h2><form id="targetForm"><input id="fbTargetDbId" type="hidden"><input id="targetName" placeholder="Название группы" required><input id="targetId" placeholder="ID или ссылка группы"><select id="targetCity"></select><label class="field">Ключевые слова<textarea id="targetKeywords" placeholder="ремонт, сантехник, работа"></textarea></label><label class="field">Слова-исключения<textarea id="targetExclude" placeholder="ищу работу, вакансия, продам"></textarea></label><label class="field">Что делать при совпадении<select id="targetAction"><option value="manual">Только записать в журнал</option><option value="facebook">Facebook / Messenger</option><option value="telegram">Telegram</option></select></label><label class="field">Материал для ответа<select id="targetMessage"></select></label><textarea id="targetNotes" placeholder="Заметки"></textarea><label><input id="targetEnabled" type="checkbox" checked> Группа включена</label><button>Сохранить группу</button><button type="button" class="ghost" onclick="resetTargetForm()">Очистить</button></form></div>

      <div class="panel full" id="messagePanel"><h2>Рекламный текст и картинка</h2><form onsubmit="saveMessage(event)"><input id="messageId" type="hidden"><div class="row"><input id="messageTitle" placeholder="Название текста" required><select id="messageAudience"><option value="all">Все</option><option value="clients">Клиенты</option><option value="workers">Мастера</option></select><label><input id="messageEnabled" type="checkbox" checked> Материал включен</label></div><label class="field">Текст рекламы<textarea id="messageBody" placeholder="Текст рекламы / ответа" required></textarea></label><div class="upload-row"><label class="field">Ссылка на картинку<input id="messageImageUrl" placeholder="Ссылка на картинку"></label><label class="field">Картинка с компьютера<input id="messageImageFile" class="native-file" type="file" accept="image/png,image/jpeg,image/webp,image/gif"><button type="button" class="file-button" onclick="messageImageFile.click()">Выберите файл</button></label></div><p id="messageImageStatus" class="meta upload-status"></p><button>Сохранить рекламный материал</button></form></div>

      <div class="panel" id="schedulePanel"><h2>Расписание</h2><form onsubmit="saveSchedule(event)"><input id="scheduleId" type="hidden"><label class="field">Город / группа<select id="scheduleCity"></select></label><label class="field">Время отправки<input id="scheduleTime" type="time" value="10:00" required></label><label class="field">Рекламный материал<select id="scheduleMessage"></select></label><label><input id="scheduleEnabled" type="checkbox" checked> Расписание включено</label><button>Сохранить время</button></form></div>

      <div class="panel"><h2>Отправить сейчас</h2><label class="field">Город<select id="sendCity"></select></label><label class="field">Рекламный материал<select id="sendMessage"></select></label><button class="success" onclick="sendNow()">Отправить сейчас</button></div>

      <div class="panel full"><h2>Что уже добавлено</h2><div id="summary" class="cards"></div></div>
      <div class="panel full" id="hitsPanel"><h2>Найденные заявки</h2><div id="hits" class="cards"></div></div>
      <div class="panel full"><h2>Журнал</h2><div id="logs" class="cards"></div></div>
    </section>
  </main>
  <script>
...
""" + MARKETING_PAGE_SCRIPT + r"""
    function resetTargetForm() {
      fbTargetDbId.value = ""; targetName.value = ""; targetId.value = ""; targetKeywords.value = ""; targetExclude.value = ""; targetNotes.value = ""; targetAction.value = "manual"; targetMessage.value = ""; targetEnabled.checked = true;
    }
    targetForm.addEventListener("submit", async event => {
      event.preventDefault();
      await api("facebook-target", {
        id: fbTargetDbId.value || null,
        name: targetName.value,
        targetId: targetId.value,
        city: targetCity.value,
        notes: targetNotes.value,
        keywords: targetKeywords.value,
        excludeKeywords: targetExclude.value,
        action: targetAction.value,
        responseMessageId: targetMessage.value,
    ...
    function editTarget(id) {
      const item = (state.targets || []).find(row => row.id === id);
      if (!item) return;
      fbTargetDbId.value = item.id;
      targetName.value = item.name || "";
      targetId.value = item.target_id || "";
      targetCity.value = item.city || "";
      targetNotes.value = item.notes || "";
      targetKeywords.value = item.keywords || "";
      targetExclude.value = item.exclude_keywords || "";
      targetAction.value = item.action || "manual";
      targetMessage.value = item.response_message_id || "";
      targetEnabled.checked = !!item.enabled;
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
    function render() {
      [targetCity, scheduleCity, sendCity].forEach(select => select.innerHTML = cityOptions(select.value));
      [targetMessage, scheduleMessage, sendMessage].forEach(select => select.innerHTML = `<option value="">${escapeHtml(mt("Не выбрано"))}</option>` + messageOptions(select.value));
      ...
      cities.innerHTML = (state.cities || []).map(city => `<span class="pill">${escapeHtml(city.name)}</span>`).join("") || "<p class='meta'>Города пока не добавлены.</p>";
      const targetsHtml = (state.targets || []).map(target => `<article class="item ${target.enabled ? "" : "off"}"><div class="item-head"><div><strong>${escapeHtml(target.name)}</strong><p class="meta">${escapeHtml(target.city || "Все города")} · ${escapeHtml(target.target_id || "ID/ссылка не указаны")}</p></div><div class="actions"><button class="secondary" onclick="editTarget(${target.id})">Редактировать</button></div></div><div>${String(target.keywords || "").split(",").filter(Boolean).map(word => `<span class="tag">${escapeHtml(word.trim())}</span>`).join("") || "<span class='meta'>Ключевые слова не заданы</span>"}</div><div>${String(target.exclude_keywords || "").split(",").filter(Boolean).map(word => `<span class="tag minus">-${escapeHtml(word.trim())}</span>`).join("")}</div><p class="meta">Действие: ${escapeHtml(target.action || "manual")} · материал #${escapeHtml(target.response_message_id || "не выбран")}</p><p>${escapeHtml(target.notes || "")}</p></article>`).join("") || "<p class='meta'>Facebook-группы пока не добавлены.</p>";
      summary.innerHTML = `
        <h3>Facebook-группы</h3>${targetsHtml}
        <h3>Рекламные материалы</h3>${(state.messages || []).map(message => `<article class="item ${message.enabled ? "" : "off"}"><strong>#${message.id} ${escapeHtml(message.title)}</strong><p>${escapeHtml(message.body)}</p>${message.image_url ? `<img class="preview" src="${escapeHtml(message.image_url)}">` : ""}<button class="secondary" onclick="editMessage(${message.id})">Редактировать</button></article>`).join("") || "<p class='meta'>Материалы пока не добавлены.</p>"}
      ...
      `;
      hits.innerHTML = (state.hits || []).map(hit => `<article class="item"><strong>${escapeHtml(hit.keyword)}</strong><p class="meta">${formatDate(hit.created_at)} · ${escapeHtml(hit.target_name || hit.username || "")}</p><p>${escapeHtml(hit.message || "")}</p></article>`).join("") || "<p class='meta'>Заявок пока нет.</p>";
      logs.innerHTML = (state.logs || []).map(log => `<article class="item"><strong>${escapeHtml(log.action)} · ${escapeHtml(log.status)}</strong><p class="meta">${formatDate(log.created_at)} · ${escapeHtml(log.city || "")} · ${escapeHtml(log.target_type)}</p><p>${escapeHtml(log.detail || "")}</p></article>`).join("") || "<p class='meta'>Журнал пока пуст.</p>";
      applyMarketingLanguage();
    }
    requireAdminAccess(loadState);
