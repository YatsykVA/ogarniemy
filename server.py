...
DATA_ROOT = os.path.abspath(
    os.environ.get("OGARNIEMY_DATA_DIR")
    or os.environ.get("PERSISTENT_DATA_DIR")
    or os.environ.get("RAILWAY_VOLUME_MOUNT_PATH")
    or os.path.join(ROOT, "data")
)
os.makedirs(DATA_ROOT, exist_ok=True)
DB_PATH = os.environ.get("TASK_DB_PATH", os.path.join(DATA_ROOT, "server.db"))
HOST = "0.0.0.0"
...
COLLECTOR_DATA_DIR = os.environ.get("COLLECTOR_DATA_DIR", os.path.join(DATA_ROOT, "collectorhub"))
MARKETING_DATA_DIR = os.path.join(DATA_ROOT, "marketing")
MARKETING_ASSETS_DIR = os.path.join(DATA_ROOT, "assets", "marketing")
os.makedirs(COLLECTOR_DATA_DIR, exist_ok=True)
os.makedirs(MARKETING_DATA_DIR, exist_ok=True)
os.makedirs(MARKETING_ASSETS_DIR, exist_ok=True)
os.environ.setdefault("TASK_DB_PATH", DB_PATH)
os.environ.setdefault("MARKETING_BOT_DB", os.path.join(MARKETING_DATA_DIR, "marketing_bot.db"))
os.environ.setdefault("MARKETING_BOT_CONFIG", os.path.join(MARKETING_DATA_DIR, "marketing_bot_config.json"))
os.environ.setdefault("COLLECTOR_DATA_DIR", COLLECTOR_DATA_DIR)
os.environ.setdefault("COLLECTOR_ENV_FILE", os.path.join(COLLECTOR_DATA_DIR, ".env"))
COLLECTOR_PROCESS = None
COLLECTOR_SEARCH_PROCESS = None
COLLECTOR_LOGIN_PROCESS = None
...
def copy_tree_contents_if_missing(source_dir, target_dir):
...
def migrate_data_to_persistent_storage():
    copy_file_if_missing(os.path.join(ROOT, "server.db"), DB_PATH)
    copy_file_if_missing(os.path.join(ROOT, "marketing_bot.db"), os.environ["MARKETING_BOT_DB"])
    copy_file_if_missing(os.path.join(ROOT, "marketing_bot_config.json"), os.environ["MARKETING_BOT_CONFIG"])
    copy_tree_contents_if_missing(os.path.join(ROOT, "assets", "marketing"), MARKETING_ASSETS_DIR)
    copy_tree_contents_if_missing(os.path.join(COLLECTOR_ROOT, "data"), COLLECTOR_DATA_DIR)
    copy_file_if_missing(os.path.join(COLLECTOR_ROOT, ".env"), os.environ["COLLECTOR_ENV_FILE"])


migrate_data_to_persistent_storage()
...
def start_marketing_bot_worker(force=False):
    global MARKETING_BOT_STARTED, MARKETING_BOT_THREAD
    if not force and not MARKETING_BOT_ENABLED:
        return
    if MARKETING_BOT_THREAD is not None and MARKETING_BOT_THREAD.is_alive():
...
def stop_marketing_bot_worker():
    ...
    try:
        import marketing_bot
        marketing_bot.stop_telegram_polling()
    except Exception as exc:
        print(f"Marketing bot stop failed: {exc}")
    MARKETING_BOT_STARTED = False


def marketing_bot_is_running():
    global MARKETING_BOT_STARTED, MARKETING_BOT_THREAD
    running = bool(MARKETING_BOT_THREAD is not None and MARKETING_BOT_THREAD.is_alive())
    MARKETING_BOT_STARTED = running
    return running
...
class App(BaseHTTPRequestHandler):
    ...
    def send_data_file(self, base_dir, relative_path, cache_seconds=0):
        file_path = os.path.abspath(os.path.join(base_dir, relative_path))
        if os.path.commonpath([os.path.abspath(base_dir), file_path]) != os.path.abspath(base_dir) or not os.path.isfile(file_path):
            self.send_json({"error": "not_found"}, 404)
            return
        with open(file_path, "rb") as file:
            body = file.read()
    ...
    def do_GET(self):
        ...
        if path in {"/styles.css", "/script.js", "/language.js", "/signup.css", "/signup.js"}:
            ...
            return
        if path.startswith("/assets/marketing/"):
            self.send_data_file(MARKETING_ASSETS_DIR, path.replace("/assets/marketing/", "", 1), 7 * 24 * 60 * 60)
            return
        ...
        if path == "/facebook-ads":
            html_path = os.path.join(ROOT, "collectorhub_project", "server_panel.html")
            self.send_html(read_text_file(html_path, FACEBOOK_ADS_HTML))
            return
        if path == "/api/tasks":
            self.handle_tasks()
            return
    ...
    def do_POST(self):
        ...
        if path == "/api/admin/telegram-login/complete":
            ...
            return
        if path == "/api/admin/telegram-login/bot-start":
            self.handle_telegram_bot_start()
            return
        if path == "/api/admin/telegram-login/bot-stop":
            self.handle_telegram_bot_stop()
            return
    ...
    def handle_collectorhub_state(self):
        ...
        env_values = {}
        env_path = os.environ["COLLECTOR_ENV_FILE"]
        for raw_line in read_text_file(env_path).splitlines():
            line = raw_line.strip()
            if line and not line.startswith("#") and "=" in line:
                key, value = line.split("=", 1)
                env_values[key.strip()] = value.strip().strip('"').strip("'")
        telegram_token = app_settings.get("telegram_bot_token") or env_values.get("TELEGRAM_BOT_TOKEN", "")
        telegram_group_id = app_settings.get("telegram_group_id") or env_values.get("TELEGRAM_GROUP_ID", "")
        settings = {
            ...
            "headless_browser": app_settings.get("headless_browser", os.name != "nt"),
            "telegram_group_id": telegram_group_id,
            "telegram_token_saved": bool(telegram_token),
            "telegram_token_mask": (telegram_token[:6] + "..." + telegram_token[-4:]) if len(telegram_token) > 12 else ("saved" if telegram_token else ""),
        }
        ...
        try:
            ...
            if path == "/api/admin/collectorhub/save-settings":
                ...
                settings = {
                    ...
                    "headless_browser": bool(data.get("headlessBrowser", os.name != "nt")),
                }
                ...
                return
            if path == "/api/admin/collectorhub/save-env":
                token = str(data.get("telegramBotToken", "") or "").strip()
                group_id = str(data.get("telegramGroupId", "") or "").strip()
                existing_env = {}
                env_path = os.environ["COLLECTOR_ENV_FILE"]
                for raw_line in read_text_file(env_path).splitlines():
                    line = raw_line.strip()
                ...
                if token:
                    existing_env["TELEGRAM_BOT_TOKEN"] = token
                if group_id:
                    existing_env["TELEGRAM_GROUP_ID"] = group_id
                env_lines = [f"{key}={value}" for key, value in sorted(existing_env.items())]
                ...
                if group_id:
                    config_data["telegram_group_id"] = group_id
                with open(config_path, "w", encoding="utf-8") as f:
                    json.dump(config_data, f, ensure_ascii=False, indent=4)
                self.send_json({"ok": True})
                return
    ...
    def handle_telegram_login_status(self):
        ...
        status["botRunning"] = marketing_bot_is_running()
    ...
    def handle_telegram_login_complete(self):
        ...
        try:
            ...
            return
        if result.get("ok"):
            start_marketing_bot_worker(force=True)
        self.send_json(result)

    def handle_telegram_bot_start(self):
        try:
            status = self.marketing().telegram_login_status()
            if not status.get("configured"):
                self.send_json({"error": "telegram_not_configured", "detail": "Telegram API ID/API Hash не настроены."}, 400)
                return
            if not status.get("sessionExists"):
                self.send_json({"error": "telegram_session_missing", "detail": "Сначала сохраните Telegram-сессию."}, 400)
                return
            start_marketing_bot_worker(force=True)
            time.sleep(0.3)
            self.send_json({"ok": True, "botRunning": marketing_bot_is_running()})
        except Exception as exc:
            self.send_json({"error": "telegram_bot_start_failed", "detail": str(exc)}, 500)

    def handle_telegram_bot_stop(self):
        try:
            was_running = marketing_bot_is_running()
            stop_marketing_bot_worker()
            time.sleep(0.3)
            self.send_json({"ok": True, "wasRunning": was_running, "botRunning": marketing_bot_is_running()})
        except Exception as exc:
            self.send_json({"error": "telegram_bot_stop_failed", "detail": str(exc)}, 500)
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .bot-box { display: grid; grid-template-columns: 1fr auto; gap: 12px; align-items: center; margin-top: 12px; padding: 12px; border: 1px solid var(--line); border-radius: 8px; background: #f8fafc; }
    .bot-actions { display: flex; flex-wrap: wrap; gap: 8px; justify-content: flex-end; }
    button.start { background: var(--green); color: white; }
    ...
    @media (max-width: 680px) { header { padding: 20px 16px; } h1 { font-size: 24px; } nav { grid-template-columns: repeat(2, minmax(0, 1fr)); } nav a { min-height: 42px; font-size: 12px; } .row, .bot-box { grid-template-columns: 1fr; } .bot-actions { justify-content: stretch; } .bot-actions button { width: 100%; } main { padding: 16px; } }
  </style>
</head>
<body>
  <header>
  ...
  <main>
    <section>
      ...
      <div class="bot-box">
        <div>
          <strong id="botStateText">Бот: проверяю...</strong>
          <div class="muted">Запуск и остановка Telegram userbot на сервере.</div>
        </div>
        <div class="bot-actions">
          <button class="start" onclick="startBot()">Запустить бот</button>
          <button class="stop" onclick="stopBot()">Остановить бот</button>
        </div>
      </div>
  ...
  <script>
    ...
    function setBotState(running) {
      botStateText.textContent = running ? "Бот: запущен" : "Бот: остановлен";
      botStateText.style.color = running ? "var(--green)" : "var(--red)";
    }
    async function loadStatus() {
      try {
        ...
        setBotState(!!data.botRunning);
        setStatus(`${configText}, ${sessionText}. Бот: ${data.botRunning ? "запущен" : "остановлен"}. Сессия: ${data.session || "ogarniemy_userbot"}${data.phone ? ", телефон: " + data.phone : ""}.`, data.sessionExists ? "ok" : "");
    ...
    async function controlBot(action) {
      const res = await fetch(`/api/admin/telegram-login/${action}`, {
        method: "POST",
      ...
      if (!res.ok) {
        setBotState(false);
        return setStatus(data.detail || "Не удалось изменить состояние Telegram userbot.", "error");
      }
      setBotState(!!data.botRunning);
      setStatus(data.botRunning ? "Telegram userbot запущен." : "Telegram userbot остановлен.", data.botRunning ? "ok" : "");
      setTimeout(loadStatus, 800);
    }
    async function startBot() {
      await controlBot("bot-start");
    }
    async function stopBot() {
      await controlBot("bot-stop");
    }
