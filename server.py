...
def start_marketing_bot_worker():
    ...
    try:
        ...
        marketing_bot = importlib.import_module("marketing_bot")
...
class App(BaseHTTPRequestHandler):
    ...
    def handle_marketing_post(self, path):
        ...
        def marketing_value(*names, default=""):
            for name in names:
        ...
        try:
            if action == "city":
                ...
                chat_id = int(str(marketing_value("chatId", "chat_id")).strip())
                watch_enabled = 1 if marketing_value("watchEnabled", "watch_enabled", default=False) else 0
                target_chat_id = str(marketing_value("targetChatId", "target_chat_id")).strip()
                ...
                if watch_enabled:
                    conn.execute(
                        ...
                        (
                            chat_id,
                            str(marketing_value("title")).strip(),
                            "",
                            str(marketing_value("keywords", "watchKeywords")).strip(),
                            str(marketing_value("excludeKeywords", "watchExcludeKeywords", "exclude_keywords")).strip(),
                            1 if marketing_value("enabled", default=True) else 0,
                            target_chat_id,
                            int(marketing_value("responseMessageId", "response_message_id") or 0) or None,
                            str(marketing_value("notes", "watchNotes")).strip(),
                            now,
                        ),
                    )
                else:
                    conn.execute(
                        ...
                        (
                            ...
                            str(marketing_value("title")).strip(),
                            1 if marketing_value("enabled", default=True) else 0,
                            str(marketing_value("notes", "ownedNotes")).strip(),
                            now,
                        ),
                    )
            ...
            conn.close()
            self.send_json({"error": "bad_value", "detail": str(exc)}, 400)
            return
        except sqlite3.Error as exc:
            conn.close()
            self.send_json({"error": "database_error", "detail": str(exc)}, 500)
            return
        conn.close()
        self.send_json({"ok": True})
...
APPROVED_MARKETING_SCRIPT = r"""
    ...
    async function saveFacebookTarget(event) {
      event.preventDefault();
      await api("facebook-target", { id:fbTargetId.value || null, name:fbTargetName.value, targetId:fbTargetLink.value, keywords:fbKeywords.value, excludeKeywords:(typeof fbExcludeKeywords !== "undefined" ? fbExcludeKeywords.value : ""), targetAction:fbAction.value, responseMessageId:fbMaterial.value, notes:fbNotes.value, enabled:fbEnabled.checked });
    ...
    async function saveMessage(event) {
      event.preventDefault();
      try { if (typeof messageImageFile !== "undefined") await imageFromFile(messageImageFile, messageImageUrl); } catch (error) { alert("Не удалось загрузить картинку."); return; }
    ...
    function editFacebookTarget(id) {
      ...
      if (!item) return;
      fbTargetId.value = item.id; fbTargetName.value = item.name || ""; fbTargetLink.value = item.target_id || ""; fbKeywords.value = item.keywords || ""; if (typeof fbExcludeKeywords !== "undefined") fbExcludeKeywords.value = item.exclude_keywords || ""; fbAction.value = item.action || "same_group"; fbMaterial.value = item.response_message_id || ""; fbNotes.value = item.notes || ""; fbEnabled.checked = !!item.enabled;
    }
