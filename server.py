...
class App(BaseHTTPRequestHandler):
    ...
    def handle_marketing_state(self, platform):
        ...
        hits = []
        if conn.execute("select name from sqlite_master where type = 'table' and name = 'facebook_keyword_hits'").fetchone():
            hits = [dict(row) for row in conn.execute(
        ...
        try:
            if action == "city":
                ...
                if platform == "telegram":
                    ...
                    table = conn.execute("select name from sqlite_master where type = 'table' and name = 'facebook_keyword_hits'").fetchone()
                    if table:
...
<body class="locked">
  ...
  <main>
    ...
    </div><aside><section class="section"><div class="section-header"><div><h2>Найденные объявления</h2><p>Последние совпадения по ключевым словам Facebook.</p></div><button class="danger" onclick="clearHits()">Очистить найденные объявления</button></div><div class="section-body"><div class="items" id="hitsList"></div></div></section><section class="section"><div class="section-header"><div><h2>Журнал Facebook</h2><p>Последние действия.</p></div><button class="danger" onclick="clearLogs()">Очистка журнала</button></div><div class="section-body"><div class="items" id="logs"></div></div></section></aside></div>
