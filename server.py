...
ADMIN_PASSWORD = os.environ.get("TASK_ADMIN_PASSWORD", secrets.token_urlsafe(24))
RESET_ADMIN_PASSWORD = os.environ.get("TASK_RESET_ADMIN_PASSWORD", secrets.token_urlsafe(24))
TRANSLATION_CACHE = {}
...
def password_matches(password, salt, expected_hash):
    return bool(password) and bool(salt) and bool(expected_hash) and hmac.compare_digest(
...
def ensure_task_columns(conn):
    ...
    if "actor_type" not in event_columns:
        conn.execute("alter table task_events add column actor_type text not null default 'employee'")
    if "actor_id" not in event_columns:
        conn.execute("alter table task_events add column actor_id integer")
    if "actor_name" not in event_columns:
        conn.execute("alter table task_events add column actor_name text not null default ''")
    if "details_json" not in event_columns:
        conn.execute("alter table task_events add column details_json text not null default '{}'")


def record_task_event(conn, task_id, event, actor_type="system", actor_id=None, actor_name="", user_id=None, details=None):
    fallback_user_id = user_id if user_id is not None else 0
    conn.execute(
        """
        insert into task_events(task_id, user_id, event, created_at, actor_type, actor_id, actor_name, details_json)
        values(?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            task_id,
...
class App(BaseHTTPRequestHandler):
    ...
    def request_language(self):
        lang = self.headers.get("X-Language", DEFAULT_LANGUAGE).strip().lower()
    ...
    def check_admin_password(self, password):
        ...
        conn.close()
        return password_matches(password, salt, stored_hash)
    ...
    def handle_login(self):
        ...
        if not row or not password_matches(password, row["salt"], row["password_hash"]):
            conn.close()
    ...
    def handle_reset_admin_password(self):
        ...
        conn.commit()
        conn.close()
        self.send_json({"ok": True, "password": RESET_ADMIN_PASSWORD})
    ...
    def handle_decision(self, path):
        ...
        if decision in ("refuse", "decline"):
            record_task_event(
                ...
                user_id=user["id"],
                details={"decision": decision, "previousStatus": row["status"], "price": row["price"]},
            )
        if decision == "refuse":
        ...
        if decision == "refuse":
            ...
            record_task_event(
                ...
                actor_name=user["display_name"],
                user_id=user["id"],
                details={"price": row["price"], "paymentMethod": normalize_payment_method(row["payment_method"])},
            )
        else:
            conn.execute(
    ...
    def create_task_from_data(self, data, client_id=None):
        ...
        if client_id is None:
            record_task_event(
                ...
                details={"price": price, "paymentMethod": payment_method},
            )
        else:
            client_row = conn.execute("select display_name from clients where id = ?", (client_id,)).fetchone()
            record_task_event(
                conn,
                task_id,
                "created_by_client",
                actor_type="client",
                actor_id=client_id,
                actor_name=client_row["display_name"] if client_row else "",
                details={"price": price, "paymentMethod": payment_method},
            )
        conn.commit()
        conn.close()
    ...
    def handle_delete_client_task(self, path):
        ...
        conn = db()
        row = conn.execute(
            "select id, status, price from tasks where id = ? and client_id = ?",
            (task_id, client["id"]),
        ).fetchone()
        if not row:
            conn.close()
        ...
        record_task_event(
            ...
            details={"status": row["status"], "price": row["price"]},
        )
        record_task_event(
            conn,
            task_id,
            ...
            actor_type="dispatcher",
            details={"status": row["status"], "assignedTo": row["assigned_to"], "price": row["price"]},
        )
    ...
    def handle_update_client_task_price(self, path):
        ...
        record_task_event(
            conn,
            task_id,
            "price_changed_by_client",
            actor_type="client",
            actor_id=client["id"],
            actor_name=client["display_name"],
            details={"oldPrice": row["price"], "newPrice": price},
        )
        conn.commit()
        conn.close()
    ...
    def handle_reset_task(self, path):
        ...
        conn = db()
        row = conn.execute("select id, status, assigned_to, price from tasks where id = ?", (task_id,)).fetchone()
        if not row:
            conn.close()
            self.send_json({"error": "task_not_found"}, 404)
            return
        ...
        record_task_event(
            ...
            "reset_by_dispatcher",
            actor_type="dispatcher",
            details={"previousStatus": row["status"], "assignedTo": row["assigned_to"], "price": row["price"]},
        )
        conn.commit()
        conn.close()
    ...
    def handle_edit_task(self, path):
        ...
        row = conn.execute(
            """
            select id, status, settlement_id, client_settlement_id, title, description, phone, address, price, payment_method, client_id
            from tasks
            where id = ?
            """,
            (task_id,),
    ...
    def handle_hide_completed_task(self, path):
        ...
        conn = db()
        row = conn.execute("select id, status, assigned_to, price from tasks where id = ? and status = 'completed'", (task_id,)).fetchone()
        if not row:
            conn.close()
            self.send_json({"error": "task_not_found"}, 404)
            return
        ...
        record_task_event(
            conn,
            task_id,
            "hidden_from_completed_by_dispatcher",
            actor_type="dispatcher",
            actor_id=row["assigned_to"],
            user_id=row["assigned_to"],
            details={"status": row["status"], "price": row["price"]},
    ...
    def build_user_report(self, user_id):
        ...
        lang = self.request_language()
        tasks = attach_task_events([task_report_json(row, lang) for row in rows])
        refused_tasks = attach_task_events([task_report_json(row, lang) for row in refused_rows])
...
def task_json(row, lang=DEFAULT_LANGUAGE):
    client_name = ""
    assigned_to_name = ""
    assigned_to_login = ""
    client_login = ""
...
def task_report_json(row, lang=DEFAULT_LANGUAGE):
    payment_method = "card"
    assigned_to_name = ""
    assigned_to_login = ""
    try:
...
def task_event_json(row):
    ...
    return {
        "event": row["event"],
        "createdAt": row["created_at"],
        "actorType": row["actor_type"] or "employee",
...
def attach_task_events(tasks):
    ...
    rows = conn.execute(
        f"""
        select task_id, event, created_at, actor_type, actor_id, actor_name, details_json
        from task_events
    ...
    events_by_task = {}
    for row in rows:
        events_by_task.setdefault(row["task_id"], []).append(task_event_json(row))
    for task in tasks:
        task["events"] = events_by_task.get(task.get("id"), [])
    return tasks
...
(function () {
  ...
  function currentLanguage() {
    const stored = localStorage.getItem("language") || "pl";
    return dict[stored] ? stored : "pl";
  }
...
HOST = "0.0.0.0"
PORT = int(os.environ.get("TASK_SERVER_PORT", "8080"))
