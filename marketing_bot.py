...
def app_path(value: str, default_name: str) -> str:
    value = str(value or "").strip()
    if not value:
        value = os.path.join(ROOT, default_name)
    elif not os.path.isabs(value):
        value = os.path.join(ROOT, value)
    return value


DB_PATH = app_path(os.environ.get("MARKETING_BOT_DB", ""), "marketing_bot.db")
CONFIG_PATH = app_path(os.environ.get("MARKETING_BOT_CONFIG", ""), "marketing_bot_config.json")
...
def config_path() -> str:
    return CONFIG_PATH if os.path.isabs(CONFIG_PATH) else os.path.join(ROOT, CONFIG_PATH)
...
def post_to_facebook_targets(text: str, target_id: str = "") -> tuple[int, int]:
    with db() as conn:
        if str(target_id or "").strip():
            try:
                target_pk = int(str(target_id).strip())
            except (TypeError, ValueError):
                target_pk = None
            rows = conn.execute(
                "select id, name, target_id from facebook_targets where enabled = 1" + (" and id = ?" if target_pk is not None else ""),
                (target_pk,) if target_pk is not None else (),
            ).fetchall()
        else:
            rows = conn.execute(
