...
DB_PATH = os.environ.get("MARKETING_BOT_DB", os.path.join(MARKETING_DATA_DIR, "marketing_bot.db"))
CONFIG_PATH = os.environ.get("MARKETING_BOT_CONFIG", os.path.join(MARKETING_DATA_DIR, "marketing_bot_config.json"))
...
TELEGRAM_SEND_INTERVAL_SECONDS = float(os.environ.get("TELEGRAM_SEND_INTERVAL_SECONDS", "12"))
...
def init_db() -> None:
    with db() as conn:
        ...
        ensure_column(conn, "facebook_targets", "exclude_keywords", "text default ''")
        ...
        ensure_column(conn, "facebook_keyword_hits", "target_id", "text default ''")
        ensure_column(conn, "facebook_keyword_hits", "target_name", "text default ''")
        ensure_column(conn, "facebook_keyword_hits", "action", "text default ''")
...
async def async_forward_telegram_event_message(
    event: Any,
    target_chat_id: int | str,
    *,
    max_wait: float = 10,
    ...
    try:
        if not await wait_for_telegram_slot(max_wait=max_wait):
            print(f"telegram userbot event forward delayed to {target_chat_id}: rate limit")
            return False
        await event.message.forward_to(int(target_chat_id))
        return True
    except Exception as exc:
        _TelegramClient, _events, _utils, _MessageMediaPhoto, FloodWaitError = load_telethon()
        if isinstance(exc, FloodWaitError):
            wait_seconds = set_telegram_flood_wait(getattr(exc, "seconds", 60))
            print(f"telegram userbot event forward delayed to {target_chat_id}: flood wait {wait_seconds} seconds")
            return False
        print(f"telegram userbot event forward failed to {target_chat_id}: {exc}")
        return False
...
def has_excluded_keyword(text: str, keywords: str | None) -> bool:
    lower = text.lower()
    return any(keyword in lower for keyword in split_keywords(keywords))
    ...
    for row in rows:
        ...
        with db() as conn:
            if ok:
                ...
                log_marketing("telegram", "outbox", row["chat_id"], "", "queued_post", "sent", f"queued #{row['id']}")
            else:
                ...
                log_marketing("telegram", "outbox", row["chat_id"], "", "queued_post", status, f"queued #{row['id']} attempt {attempts}")
...
def run_due_schedules() -> None:
    ...
    for schedule in due_schedules("facebook"):
        sent, total = post_to_facebook_targets(schedule["body"], schedule["target_id"] or "")
        with db() as conn:
            conn.execute(
                "update marketing_schedules set last_sent_date = ? where id = ?",
                (today, schedule["id"]),
...
async def handle_group_message(message: dict[str, Any]) -> None:
    ...
    if not keywords:
        return
    if config and has_excluded_keyword(text, config["exclude_keywords"]):
        return
    for keyword in keywords:
        if keyword in lower:
            if can_process_keyword_message(chat_id, message.get("message_id"), keyword):
                ...
                if target_chat_id:
                    event = message.get("_event")
                    if event:
                        ok = await async_forward_telegram_event_message(event, reply_chat_id)
                    else:
...
async def run_telegram_userbot() -> None:
    global TELEGRAM_USERBOT_LOOP, TELEGRAM_USERBOT_STOP_REQUESTED
    init_db()
    TELEGRAM_USERBOT_STOP_REQUESTED = False
    ...
    try:
        ...
        async def on_new_message(event: Any) -> None:
            try:
                if event.out and not event.is_channel and not (event.raw_text or "").strip().startswith("/"):
                    return
        ...
        globals()["TELEGRAM_USERBOT_CLIENT"] = None
        globals()["TELEGRAM_USERBOT_LOOP"] = None


def stop_telegram_polling() -> bool:
    global TELEGRAM_USERBOT_STOP_REQUESTED
    TELEGRAM_USERBOT_STOP_REQUESTED = True
    client = TELEGRAM_USERBOT_CLIENT
    loop = TELEGRAM_USERBOT_LOOP
    if not client or not loop or not loop.is_running():
        return False
...
def facebook_page_access_token() -> str:
    return os.environ.get("FACEBOOK_PAGE_ACCESS_TOKEN", "").strip() or str(read_db_config().get("facebook_page_access_token", "")).strip()


def facebook_send(psid: str, text: str) -> bool:
    global FACEBOOK_LAST_SEND_ERROR
    FACEBOOK_LAST_SEND_ERROR = ""
    token = facebook_page_access_token()
    if not token:
        FACEBOOK_LAST_SEND_ERROR = "missing FACEBOOK_PAGE_ACCESS_TOKEN"
        return False
    ...
    try:
        ...
        FACEBOOK_LAST_SEND_ERROR = str(exc)
        print(f"facebook send failed for {psid}: {FACEBOOK_LAST_SEND_ERROR}")
        return False


def post_to_facebook_targets(text: str, target_id: str = "") -> tuple[int, int]:
    with db() as conn:
        if target_id:
            rows = conn.execute(
                "select id, name, target_id from facebook_targets where enabled = 1 and id = ?",
                (int(target_id),),
            ).fetchall()
        else:
            rows = conn.execute(
                "select id, name, target_id from facebook_targets where enabled = 1"
            ).fetchall()
    sent = 0
    for row in rows:
        facebook_id = str(row["target_id"] or "").strip()
        ok = bool(facebook_id and facebook_send(facebook_id, text))
        if ok:
            sent += 1
        detail = f"{row['name'] or row['id']} -> {facebook_id or 'missing target_id'}"
        if FACEBOOK_LAST_SEND_ERROR:
            detail += f" ({FACEBOOK_LAST_SEND_ERROR[:500]})"
        log_marketing("facebook", "target", row["id"], "", "post", "sent" if ok else "failed", detail)
    return sent, len(rows)


def default_telegram_forward_chat_id() -> str:
    configured = str(read_db_config().get("facebook_forward_telegram_chat_id", "")).strip()
    if configured:
        return configured
    with db() as conn:
        row = conn.execute(
            "select chat_id from telegram_groups where enabled = 1 order by title, chat_id limit 1"
        ).fetchone()
    return str(row["chat_id"]) if row else ""


def facebook_forward_target_id() -> str:
    return str(read_db_config().get("facebook_forward_target_id", "")).strip()


def facebook_found_ad_text(target_name: str, keyword: str, sender: str, text: str) -> str:
    lines = [
        f"Найдено Facebook-объявление по ключевому слову: {keyword}",
        "",
        f"Источник: {target_name or 'Facebook'}",
        f"Автор/ID: {sender or 'не указан'}",
        "",
        "Текст объявления:",
        text.strip() or "(без текста)",
    ]
    return "\n".join(lines)[:4096]


def record_facebook_keyword_hit(target: sqlite3.Row, sender: str, keyword: str, message_text: str) -> None:
    with db() as conn:
        conn.execute(
            """
            insert into facebook_keyword_hits(target_id, target_name, keyword, user_id, username, message, action, created_at)
            values(?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                sender,
                message_text[:1000],
                str(target["action"] or ""),
                int(time.time()),
            ),
        )


def handle_facebook_keyword_matches(sender: str, message_text: str) -> bool:
    ...
    with db() as conn:
        targets = conn.execute(
            "select id, name, target_id, keywords, exclude_keywords, action, response_message_id from facebook_targets where enabled = 1"
        ).fetchall()
    for target in targets:
        target_id = str(target["target_id"] or "").strip()
        if has_excluded_keyword(clean_text, target["exclude_keywords"]):
            continue
        keywords = split_keywords(target["keywords"])
        for keyword in keywords:
            if keyword not in lower:
                continue
            record_facebook_keyword_hit(target, sender, keyword, clean_text)
            action = str(target["action"] or "same_group").strip()
            notification = facebook_found_ad_text(str(target["name"] or ""), keyword, sender, clean_text)
            if action == "forward_telegram":
                telegram_chat_id = default_telegram_forward_chat_id()
                ok = bool(telegram_chat_id and send_telegram(int(telegram_chat_id), notification))
                log_marketing("facebook", "target", target_id or sender, "", "keyword_forward_telegram", "sent" if ok else "failed", f"to {telegram_chat_id or 'missing telegram group'}, keyword: {keyword}")
            elif action == "forward_facebook":
                facebook_target = facebook_forward_target_id()
                ok = bool(facebook_target and facebook_send(facebook_target, notification))
                log_marketing("facebook", "target", target_id or sender, "", "keyword_forward_facebook", "sent" if ok else "failed", f"to {facebook_target or 'missing facebook target'}, keyword: {keyword}")
            elif action == "same_group":
                response = marketing_message(int(target["response_message_id"] or 0) or None, "facebook")
                reply_text = str(response["body"] or "") if response else GROUP_TEXT
                ok = facebook_send(sender, reply_text)
                log_marketing("facebook", "target", target_id or sender, "", "keyword_reply", "sent" if ok else "failed", f"keyword: {keyword}")
            else:
                log_marketing("facebook", "target", target_id or sender, "", "keyword_manual", "saved", f"keyword: {keyword}")
            return True
    return False
...
def handle_facebook_event(event: dict[str, Any]) -> None:
    ...
    raw_message_text = (event.get("message", {}).get("text") or "").strip()
    ...
    if handle_facebook_keyword_matches(sender, raw_message_text):
        return
